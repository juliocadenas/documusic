...
model = (AutoModelForCausalLM.from_pretrained(
    stage1_model, attn_implementation="eager", load_in_8bit=True, device_map="auto",
) if os.environ.get('YUE_USE_8BIT', '1') == '1' else
AutoModelForCausalLM.from_pretrained(
    stage1_model, attn_implementation="eager", torch_dtype=torch.bfloat16,
))
# to device, if gpu is available
if os.environ.get('YUE_USE_8BIT', '1') != '1': model.to(device)  # [DocuMusic] conditional .to(device)
model.eval()
...
model_stage2 = (AutoModelForCausalLM.from_pretrained(
    stage2_model, attn_implementation="eager", load_in_8bit=True, device_map="auto",
) if os.environ.get('YUE_USE_8BIT', '1') == '1' else
AutoModelForCausalLM.from_pretrained(
    stage2_model, attn_implementation="eager", torch_dtype=torch.bfloat16,
))
if os.environ.get('YUE_USE_8BIT', '1') != '1': model_stage2.to(device)  # [DocuMusic] conditional .to(device)
model_stage2.eval()
